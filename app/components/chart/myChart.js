...
const MyChart = withTranslation()(class Trends extends PureComponent {
  ...
  handleClickMyChart(e) {
    ...
    // when you're on MyChart view, clicking MyChart does nothing
    return;
  }
  //@t do my own version; does not seem that i have to do it for every page, just for handleClickBhLog below
  handleClickTrends(e) {
    if (e) {
      e.preventDefault();
    }
  ...
  // @t do I need to touch the onSwitchToBgLog prop? search it
  handleClickBgLog(e) {
    if (e) {
      e.preventDefault();
    }
  ...
  render() {
    ...
    const dataQueryComplete = _.get(this.props, 'data.query.chartType') === 'myChart';
